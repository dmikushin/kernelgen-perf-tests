...
extern "C" __global__
#endif
void vecadd(int nx, int ny, int ns,
#if defined(__CUDACC__)
...
#endif
	for (int k = 0 + k_offset; k < ns; k += k_increment)
	{
#if defined(_OPENACC)
		#pragma acc loop independent
#endif
		for (int j = 0 + j_offset; j < ny; j += j_increment)
		{
#if defined(_OPENACC)
			#pragma acc loop independent vector(512)
#endif
			for (int i = 0 + i_offset; i < nx; i += i_increment)
			{
...
#else
			vecadd<<<config.gridDim, config.blockDim, config.szshmem>>>(
				nx, ny, ns,
				config,
				w0p, w1p, w2p);
